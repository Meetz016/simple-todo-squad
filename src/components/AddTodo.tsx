...
export const AddTodo = ({
  ...
  return <form onSubmit={handleSubmit} className="flex gap-3 mb-6">
      <div className="flex-1 relative">
        <Input value={text} onChange={e => setText(e.target.value)} placeholder="Add a new todo..." className={`pr-12 bg-gradient-card border-border focus:border-primary focus:ring-1 focus:ring-primary text-foreground placeholder:text-muted-foreground ${text.length > 0 ? 'bg-blue-600' : 'bg-purple-600'}`} />
      </div>
